<?php
    ...
    $buscar_venda .= "WHERE codigo_venda = '{$cod_venda}' ";
    ...
    $pagamento_id = $resultado_venda['pagamento_id'];
    $cpf = $resultado_venda['cpf'];
    $quantidade = $resultado_venda['quantidade'];
    $valor = $resultado_venda['valor'];
    $registrado = $resultado_venda['registrado'];
    $data_venda = $resultado_venda['data_venda'];
    $entrega_id = $resultado_venda['entrega_id'];
    $observacao = $resultado_venda['observacao'];

    $buscar_nome_v = "SELECT nome ";
    $buscar_nome_v .= "FROM funcionarios ";
    $buscar_nome_v .= "WHERE registro = {$registro} ";
    $buscando_nome_v = mysqli_query($conecta,$buscar_nome_v);
    $resultado_nome_vendedor = mysqli_fetch_assoc($buscando_nome_v);

    $nome_vendedor = utf8_encode($resultado_nome_vendedor['nome']);

    $buscar_loja = "SELECT * ";
    $buscar_loja .= "FROM locais ";
    $buscar_loja .= "WHERE local_id = {$local_id} ";
    $buscando_loja = mysqli_query($conecta,$buscar_loja);
    $resultado_local_id = mysqli_fetch_assoc($buscando_loja);

    $nome_loja = utf8_encode($resultado_local_id['nome']);
    $endereco = utf8_encode($resultado_local_id['endereco']);
    $numero = $resultado_local_id['numero'];
    $complemento = utf8_encode($resultado_local_id['complemento']);
    $cidade = utf8_encode($resultado_local_id['cidade']);
    $estado_id = $resultado_local_id['estado_id'];
    $telefone = utf8_encode($resultado_local_id['telefone']);
...
<html>
    <head>
        ...
        <title>Nota de Venda | Kodok</title>
    ...
    <body>
        ...
        <div class="container-fluid">
            <div class="box">
                <div class="row">
                    <div class="col-6">
                        <h3>Cupom (2ª via)</h3>
                        <br><h5><?php echo $cod_venda ?></h5>
                        <h6>Destinatário: <?php echo utf8_encode($observacao) ?></h6>
                    </div>
                    <div class="col-6 right" style="float: right">
                        <h6>Vendido por: <?php echo $nome_vendedor ?></h6>
                        <h6>Loja: <?php echo $nome_loja ?></h6>
                        <h6><?php echo $endereco.', '.$numero.' '.$complemento.', '.$cidade.' - '.$estado_id;  ?></h6>
                        <h6>Total de itens: <?php echo $quantidade ?></h6>
                        <h6>Valor total da mercadoria: <?php echo 'R$ ' . number_format($valor, 2, ',', '.'); ?></h6>
                    </div>
                </div>
                <div class="row estoque">
                    ...
                    <?php
                        ...
                        $listar_vendas .= "WHERE codigo_venda = '{$cod_venda}' ";
        ...
        <div id="noprint" class="container-fluid row col-12">
            ...
            <div class="col-12 col-lg-3">
                <a href="todas-vendas.php" class="btn btn-block">Fechar</a>
            </div>
        </div>
    </body>
</html>