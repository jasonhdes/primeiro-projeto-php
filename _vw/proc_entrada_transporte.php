<?php
    ...
    if(($resultado_model) AND ($resultado_model->num_rows != 0 )){
        while($row_model = mysqli_fetch_assoc($resultado_model)){
            ...
            echo "<form class='col-12' action='entrada-transporte.php' method='post'>";
            echo "<div class='row'>";
            echo "<div class='col-12 col-lg-3'>";
            echo "<label for='local'>Local de Entrada:</label>";
            echo "<select name='local' id='local' class='form-control'>";
                    while($lista_locais = mysqli_fetch_assoc($selecionando_local)){
                        echo "<option value='".$lista_locais['local_id']."'>".utf8_encode($lista_locais['nome'])."</option>";
                    }
            echo "</select>";
            echo "</div>";
